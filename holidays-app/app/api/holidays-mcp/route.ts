...
export async function POST(request: Request) {
  ...
  const { date, systemPrompt, userPrompt } = requestJson as {
    date?: string;
    systemPrompt?: string;
    userPrompt?: string;
  };
  ...
  void (async () => {
    try {
      ...
      try {
        ...
        await writeEvent("status", "Warning: MCP server offline/unreachable. Falling back to direct model query.");
      }
      ...
      if (toolCalls && toolCalls.length > 0) {
        ...
        await streamCloudflareAiResponse(finalResponse, writeEvent);
        await close();
        return;
      }
      ...
      console.error("Error executing background task:", error instanceof Error ? error.stack : error);
