...
export async function POST(request: Request) {
  ...
  // Read request parameters first
  let requestJson: any;
  try {
    requestJson = await request.json();
  } catch (err) {
    void (async () => {
      await writeError("Invalid JSON request body.");
    })();
    return response;
  }

  const { userPrompt, systemPrompt } = requestJson as {
    userPrompt?: string;
    systemPrompt?: string;
  };
...
Example JSON output: {"semantic_query": "public holiday", "start_date": "2026-06-02", "end_date": "2026-06-02", "countries": []}`;
      ...
      if (filterMetadata && typeof filterMetadata === "object") {
        if (!filterMetadata.start_date && !filterMetadata.end_date) {
          try {
            ...
            console.error("Manual fallback parsing failed:", e);
          }
        }
      }

      if (!filterMetadata.semantic_query) {
        filterMetadata.semantic_query = "public holiday";
      }

      console.log("Extracted search metadata:", JSON.stringify(filterMetadata));
      await writeEvent("status", `Extracted query criteria: "${filterMetadata.semantic_query}" in dates ${filterMetadata.start_date || 'any'} to ${filterMetadata.end_date || 'any'}.`);

      // Step 2: Query embedding...
      await writeEvent("status", "Generating search embeddings...");
      ...
      await streamCloudflareAiResponse(synthesisRes, writeEvent);
      await close();

    } catch (error) {
      console.error("Error executing background task in RAG:", error instanceof Error ? error.stack : error);
